...
Yeelight.startmusicservice = function (list) {
  ...
  discover.on('response', function (response) {
    ...
    console.log(response.headers.name + ' : ' + address);
  ...
  function setmusic() {
    for (var name in Yeelight.lights) {
      if (!Yeelight.lights[name].socket) {
        ...
        Yeelight.lights[name].client.connect(Yeelight.lights[name].port, Yeelight.lights[name].address, (function (nname) {
          return function () {
            Yeelight.lights[nname].client.write(commandstring('set_music', [1, Yeelight.lights[nname].client.localAddress, 5000]));
          };
        })(name));

        Yeelight.lights[name].client.on('data', (function (nname) {
          return function (data) {
            //console.log( nname+': Received: ' + data);
            Yeelight.lights[nname].client.destroy(); // kill client after server's response
          };
        })(name));

        Yeelight.lights[name].client.on('error', (function (nname) {
          return function (err) {
            //console.log( nname+': error: ' + err.message);
            Yeelight.lights[nname].client.destroy(); // kill client after server's response
          };
        })(name));
      }
