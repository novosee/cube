...
function initmusicserver() {
  ...
  net.createServer(function serverCb(socket) {
    ...
    for (var id in Yeelight.lights) {
      if (Yeelight.lights[id].address == socket.remoteAddress) {
        Yeelight.lights[id].socket = socket;
        console.log(Yeelight.lights[id].address + '   connected!!!!');
    ...
    socket.on('end', function () {
      //Yeelight.lights[id].socket = null;
      console.log(Yeelight.lights[id].address + " left!\n");
    });
    socket.on('error', function () {
      //Yeelight.lights[id].socket = null;
      console.log(Yeelight.lights[id].address + " error!\n");
    });


  }).listen(5000, '192.168.1.20');
...
Yeelight.startmusicservice = function (list) {
  ...
  discover.on('response', function (response) {
    ...
    if (address) {
      ...
      Yeelight.lights[response.headers.id] = Yeelight.lights[response.headers.id] || {};
      var u = url.parse(address);
      if (u.protocol === 'yeelight:') {
        Yeelight.lights[response.headers.id].address = u.hostname;
        Yeelight.lights[response.headers.id].port = u.port || 55443;
      }


      Yeelight.lights[response.headers.id].client = new tcp.Socket();
      Yeelight.lights[response.headers.id].client.connect(Yeelight.lights[response.headers.id].port, Yeelight.lights[response.headers.id].address, function () {
        console.log(response.headers.id + ':  set music  ' + commandstring('set_music', [1, Yeelight.lights[response.headers.id].client.localAddress, 5000]));
        Yeelight.lights[response.headers.id].client.write(commandstring('set_music', [1, Yeelight.lights[response.headers.id].client.localAddress, 5000]));
      });
      Yeelight.lights[response.headers.id].client.on('data', function (data) {
        console.log(response.headers.id + ': Received: ' + data);
        Yeelight.lights[response.headers.id].client.destroy(); // kill client after server's response
      });
      Yeelight.lights[response.headers.id].client.on('close', function () {
        console.log(response.headers.id + ':   Connection closed');
      });
      console.log('address:' + Yeelight.lights[response.headers.id].address + '        port:  ' + Yeelight.lights[response.headers.id].port);
...
Yeelight.rgbcommand = function (id, rgb_value, effect, duration) {
  if (Yeelight.lights[id]) {
    if (Yeelight.lights[id].socket) {

      Yeelight.lights[id].socket.write(commandstring('set_rgb', [rgb_value, effect || 'smooth', duration || 500]));
    }
  }

};

Yeelight.cfcommand = function (id, count, action, flow_expression) {
  if (Yeelight.lights[id]) {

    if (Yeelight.lights[id].socket) {
      console.log(commandstring('start_cf', [count, action, flow_expression]));

      Yeelight.lights[id].socket.write(commandstring('start_cf', [count, action, flow_expression]));
    }
  }
};

Yeelight.scenecommand = function (id, params) {
  if (Yeelight.lights[id]) {
    if (Yeelight.lights[id].socket) {
      console.log(commandstring('set_scene', [params]));
      Yeelight.lights[id].socket.write(commandstring('set_scene', ["cf", 1, 1, params]));
    }
  }
};
...
Yeelight.discover = function (callback) {
  ...
  discover.on('response', function (response) {
    ...
    if (address) {
      ...
      console.log('address:' + address + '        id:  ' + response.headers.id);
