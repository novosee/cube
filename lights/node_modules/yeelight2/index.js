...
Yeelight.startmusicservice = function (list) {
  ...
  discover.on('response', function (response) {
    ...
    if (address) {
      ...
      Yeelight.lights[response.headers.name].client = new tcp.Socket();
      Yeelight.lights[response.headers.name].client.connect(Yeelight.lights[response.headers.name].port, Yeelight.lights[response.headers.name].address, function () {
        console.log(response.headers.name + ':  set music  ' + commandstring('set_music', [1, Yeelight.lights[response.headers.name].client.localAddress, 5000]));
        Yeelight.lights[response.headers.name].client.write(commandstring('set_music', [1, Yeelight.lights[response.headers.name].client.localAddress, 5000]));
      });
      Yeelight.lights[response.headers.name].client.on('data', function (data) {
        console.log(response.headers.name + ': Received: ' + data);
        Yeelight.lights[response.headers.name].client.destroy(); // kill client after server's response
      });
      Yeelight.lights[response.headers.name].client.on('close', function () {
        console.log(response.headers.name + ':   Connection closed');
      });
      console.log('address:' + Yeelight.lights[response.headers.name].address + '        port:  ' + Yeelight.lights[response.headers.name].port);
